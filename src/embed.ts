...
export function embedBlock(
  md: MarkdownIt,
  options?: { embedUrls?: Record<string, string> | ((url: string) => string) }
) {
  const open = '{% embed'
  const lineClose = '%}'
  ...
  md.renderer.rules.embed = (tokens, idx) => {
    ...
    if (!iframeUrl || !(iframeUrl.startsWith('<') && iframeUrl.endsWith('>'))) {
      iframeUrl = `<iframe src="${iframeUrl || url}"${
        caption ? ` alt="${caption}"` : ''
      } style="padding:16px 0;width:100%;max-with:100%;border:none;"></iframe>`
    }
    return `<figure class="gitbook-embed${caption ? ' with-caption' : ''}">\n  ${iframeUrl}\n${
      caption
