...
export function swaggerBlock(
  md: MarkdownIt,
  options?: { swaggerUrls?: Record<string, string> | ((url: string) => string) }
) {
  const open = '{% swagger'
  const lineClose = '%}'
  const blockClose = '{% endswagger %}'
  const swaggerBlock: ParserBlock.RuleBlock = (state, startLine, endLine, silent) => {
    ...
    for (;;) {
      ...
      // swagger end look-down should not over 2 lines
    ...
    const token = state.push('swagger', haveBlockEndMarker ? 'swagger_block' : 'swagger', 0)
    token.block = true
  ...
  md.block.ruler.after('blockquote', 'swagger', swaggerBlock, {
    alt: ['paragraph', 'reference', 'blockquote', 'list']
  })
  md.renderer.rules.swagger = (tokens, idx) => {
